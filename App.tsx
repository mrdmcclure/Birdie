  ...
  try {
    ...
    if (firstBrace !== -1 && lastBrace !== -1) {
      try {
        ...
        console.error("Failed to parse substring JSON:", text.substring(firstBrace, lastBrace + 1));
      }
    }
...
const App: React.FC = () => {
  ...
  const handleSend = async (messageText: string, e?: React.FormEvent) => {
    ...
    try {
      ...
      const data: GeminiResponse = extractJSON(response.text);
      ...
      if (data.followUpQuestion) {
        const isActionable = !data.followUpQuestion.toLowerCase().includes("which bird species");
        setMessages(prev => [...prev, {
  ...
  return (
    <div classname="flex flex-col h-screen max-w-4xl mx-auto bg-white shadow-2xl">
      ...
      {/* Chat Area */}
      <div ref="{scrollRef}" classname="flex-1 overflow-y-auto p-4 space-y-4 bg-emerald-50/30 scroll-smooth">
        {messages.map((msg, index) => {
          const isLastMessage = index === messages.length - 1;
          const showActions = isLastMessage && msg.role === 'bot' && msg.isActionable && !isLoading;

          return (
            <div key="{msg.id}" classname="{`flex" ${msg.role="==" 'user'="" ?="" 'justify-end'="" :="" 'justify-start'}="" animate-in="" fade-in="" slide-in-from-bottom-2="" duration-300`}="">
              <div classname="{`max-w-[85%]" rounded-2xl="" p-4="" shadow-sm="" relative="" ${="" msg.role="==" 'user'="" ?="" 'bg-emerald-600="" text-white="" rounded-tr-none="" shadow-emerald-200'="" :="" 'bg-white="" text-gray-800="" border="" border-emerald-100="" rounded-tl-none'="" }`}="">
                ...
                {msg.imageUrl && (
                  <div classname="mt-4 overflow-hidden rounded-xl border-4 border-white ring-1 ring-emerald-100 shadow-xl group">
                    <img src="{msg.imageUrl}" alt="Silly Bird" classname="w-full h-auto object-cover transform transition-transform group-hover:scale-105"/>
                ...
                {showActions && (
                  <div classname="mt-4 flex flex-wrap gap-2">
                    <button onclick="{()" ==""> handleSend("Another image from this story please!")}
                    ...
                    <button onclick="{()" ==""> handleSend("Let's do a different story with a new bird!")}
      ...
      <div classname="p-4 bg-emerald-800 border-t border-emerald-900 sticky bottom-0 shadow-[0_-10px_20px_rgba(0,0,0,0.1)]">
        <form onsubmit="{(e)" ==""> handleSend(input, e)} className="flex gap-2">
          <input type="text" value="{input}" onchange="{(e)" ==""> setInput(e.target.value)}
            ...
            disabled={isLoading}
          />
          <button type="submit" disabled="{isLoading" ||="" !input.trim()}="" classname="bg-emerald-500 text-white px-8 py-3.5 rounded-full font-bold hover:bg-emerald-400 active:scale-95 transition-all shadow-lg disabled:opacity-50 disabled:shadow-none disabled:scale-100">
            Send
          </button>
        </form>
